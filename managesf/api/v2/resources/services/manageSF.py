...
def prepare_resources_data(raw_resources, type='project'):
    ...
    if type == 'project':
        ...
        for project in raw_projects:
            p = {}
            p['name'] = raw_projects[project].get('name', project)
            p['repository'] = \
            ...
            p['mailing_list'] = raw_projects[project].get('mailing-lists') or\
                []
            p['contact'] = raw_projects[project].get('contact') or []
            p_acl = {}
            p_members = {}
            for repo in p['repository']:
                ...
                for p_group in p_groups:
                    for m in raw_groups[p_group]['members']:
                        p_members[m] = 0
            p['acl'] = p_acl.keys() or []
            p['member_email'] = p_members.keys() or []
            projects[project] = p
        return projects
    if type == 'acl':
        acls = {}
        for acl in raw_acls:
            ...
            a['group'] = raw_acls[acl].get('groups') or []
            a_projects = {}
            a_repositories = {}
            a_members = {}
            ...
            for rep in raw_repositories:
                if raw_repositories[rep].get('acl') == acl:
                    ...
                    for p in prj:
                        a_projects[p] = 0
            a['member_email'] = a_members.keys() or []
            a['project'] = a_projects.keys() or []
            a['repository'] = a_repositories.keys() or []
            acls[acl] = a
        return acls
...
class ProjectManager(resources.ProjectManager):
    ...
    def get(self, **kwargs):
        ...
        # list-type properties: repository, mailing_list, contact, member_email

        def _l(k):
            return lambda x: kwargs[k] in projects[x].get(k, [])

        for k in ['repository', 'mailing_list', 'contact', 'member_email', ]:
            if kwargs.get(k):
        ...
        for p in projects:
            if all(f(p) for f in predicates):
                ...
                pj = resources.Project(id=p, name=_p.get('name'),
                                       ...
                                       repositories=repos)
                results.append(pj)
        return sorted(results, key=attrgetter(kwargs.get('order_by')),
...
class ACLManager(resources.ACLManager):
    ...
    def get(self, **kwargs):
        ...
        if kwargs.get('id'):
            predicates.append(lambda x: x == kwargs['id'])
        # list-type properties: repository, project, group, member_email

        def _l(k):
            return lambda x: kwargs[k] in acls[x].get(k, [])

        for k in ['repository', 'project', 'group', 'member_email', ]:
            if kwargs.get(k):
                predicates.append(_l(k))
        for a in acls:
...
class GroupManager(resources.GroupManager):
    ...
    def get(self, **kwargs):
        ...
        engine = self.manager.get_engine('read')
        raw_resources = engine.get(self.manager.master_repo, 'master')
        groups = prepare_resources_data(raw_resources, 'group')
        predicates = []
        # initialize predicates for empty query
        predicates.append(lambda x: True)
        # id
        if kwargs.get('id'):
            predicates.append(lambda x: x == kwargs['id'])
        # name, acl
        # list-type properties: repository, project, acl, member_email

        def _l(k):
            return lambda x: kwargs[k] in groups[x].get(k, [])

        for k in ['repository', 'project', 'acl', 'member_email', ]:
            if kwargs.get(k):
                predicates.append(_l(k))
        for g in groups:
            if all(f(g) for f in predicates):
                _g = groups[g]
                gg = resources.Group(id=g, name=_g.get('name'),
