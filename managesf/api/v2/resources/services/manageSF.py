...
class ResourcesCache:
    ...
    def cache(self):
        self.load_tables()
        return self.engine, self._tables, self._raw_resources

    def load_tables(self):
        rsc_engine = self.manager.get_engine('read')
        raw_resources = rsc_engine.get(self.manager.master_repo, 'master')
        if self.current_hash == raw_resources['hash']:
            return
        ...
        self.current_hash = raw_resources['hash']
...
class ProjectManager(resources.ProjectManager):
    ...
    def get(self, **kwargs):
        results = []
        engine, tables, raw_resources = self.manager.resource_cache.cache
...
class SFResourcesManager(resources.ResourcesServiceManager):
    ...
    def __init__(self, conf):
        ...
        self.resource_cache = ResourcesCache(self)
