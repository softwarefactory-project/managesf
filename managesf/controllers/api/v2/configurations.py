...
class ZuulConfigurationController(base.APIv2RestController):
    @expose()
    def get(self, **kwargs):
        return ZuulTenantsLoad().start()


class ConfigurationController:
    zuul = ZuulConfigurationController()


class ZuulTenantsLoad:
    def __init__(self, url=None):
        if url is None:
            # TODO(fbo): use direct internal resources lib if call from
            # ZuulConfigurationController. Cli can still use the managesf REST
            # call
            url = "http://localhost:20001/v2"
        self.url = url

    def get_resources(self, url, verify_ssl=True):
        url = os.path.join(url, "resources")
    ...
    def merge_tenant_from_resources(
        ...
        for pid, project in tenant_resources['resources']['projects'].items():
            ...
            for sr in project['source-repositories']:
                ...
                sr_name = sr.keys()[0]
                if sr_name in projects_list[tenant_name]:
                ...
                tenants[tenant_name]['source'][source].setdefault(
    ...
    def start(self):
        resources = self.get_resources(self.url)
        tenants = {}
        projects_list = {}
        for tenant_name, data in resources.get(
                "resources", {}).get("tenants", {}).items():
            tenant_resources = self.get_resources(data["url"])
