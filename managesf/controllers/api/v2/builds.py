...
class BuildController(RestController):
    ...
    def get(self, **kwargs):
        ...
        if 'started_before' in kwargs:
            try:
                kwargs['started_before'] = datetime.fromtimestamp(
                    float(kwargs['started_before']))
            except:
                response.status = 400
                return {'error_description': "timestamp must be an int",
                        'traceback': traceback.format_exc()}
        if 'started_after' in kwargs:
            try:
                kwargs['started_after'] = datetime.fromtimestamp(
                    float(kwargs['started_after']))
            except:
                response.status = 400
                return {'error_description': "timestamp must be an int",
                        'traceback': traceback.format_exc()}
