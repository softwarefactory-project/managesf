...
from managesf.controllers.api.v2 import nodepool as v2_np
...
class NodesController(RestController):
    ...
    class DIBImageController(RestController):
        @expose('json')
    ...
    class DIBImageUpdateController(RestController):
        @expose('json')
        def put(self, image_name, **kwargs):
            ...
            provider = AGENTSPROVIDERS[0]
            try:
                update_id = provider.dib_image.start_update(image_name)
                response.status = 201
        ...
        @expose('json')
        def post(self, image_name, provider_name, **kwargs):
            """upload the image to a provider"""
            _policy = 'managesf.node:image-start-update'
            if not authorize(_policy,
                             target={"image": image_name,
                                     "provider": provider_name}):
            ...
            provider = AGENTSPROVIDERS[0]
            try:
                update_id = provider.dib_image.start_upload(image_name,
                                                            provider_name)
                response.status = 201
                d = {provider.service_name: {'update_id': update_id}}
            except Exception as e:
        ...
        def get(self, id):
            ...
            provider = AGENTSPROVIDERS[0]
            info = provider.dib_image.get_action_info(id)
            if not info:
                return abort(404)
            if not authorize(_policy,
    ...
    class LegacyImageUpdateController(RestController):
        @expose('json')
        def put(self, image_name, provider_name, **kwargs):
            _policy = 'managesf.node:image-start-update'
            if not authorize(_policy,
                             target={"image": image_name,
                                     "provider": provider_name}):
            ...
            provider = AGENTSPROVIDERS[0]
            try:
                update_id = provider.image.start_update(image_name,
                                                        provider_name)
                response.status = 201
                d = {provider.service_name: {'update_id': update_id}}
            except Exception as e:
        ...
        @expose('json')
        def post(self, image_name, provider_name, **kwargs):
            return self.put(image_name, provider_name, **kwargs)

        @expose('json')
        def get(self, id):
            _policy = 'managesf.node:image-update-status'
            provider = AGENTSPROVIDERS[0]
            info = provider.image.get_action_info(id)
            if not info:
                return abort(404)
            if not authorize(_policy,
                             target={"image": info['image'],
    ...
    images = LegacyImageController()
    images.update = LegacyImageUpdateController()
    dib_images = DIBImageController()
    dib_images.update = DIBImageUpdateController()
...
class V2Controller(object):
    ...
    zuul.admin = v2_zuul.ZullAdminController()
