...
class SFGerritUserManager(base.UserManager):
    ...
    def create(self, username, email, full_name, ssh_keys, cauth_id):
        ...
        self.log.debug("Creating account %s", username)
        ...
        if ssh_keys_valid:
            try:
                ...
                self.log.error("Couldn't extract ssh key from %s" % ssh_keys)
        try:
    ...
    def get(self, username):
        if not self.plugin.conf.get("new_gerrit_client"):
            ...
            try:
                ...
                pass
            return None
        self.log.debug("Getting account %s", username)
        try:
            client = self.plugin.get_client()
    ...
    def update(self, uid, username=None, full_name=None, email=None,
        ...
        self.log.debug("Updating account %s")
        client = self.plugin.get_client()
    ...
    def delete(self, username):
        ...
        client = self.plugin.get_client()
        try:
            account_id = client.get_account(username).get('_account_id')
        except Exception:
            self.log.exception(u"Account %s not found, skip deletion",
                               username)
            return
        if self.plugin.conf.get("new_gerrit_client"):
            self.log.warning("Couldn't delete the user, need manual intervention")
            return
