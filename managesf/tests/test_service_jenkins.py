...
class TestSFJenkinsManager(BaseSFJenkinsService):
    def test_get_job(self):
        patches = [
            patch.object(SFJenkins, 'jenkins_open'),
        ]
        with nested(*patches) as (jenkins_open, ):
            ...
            self.assertEqual('SUCCESS',
                             jobs[0]['status'])
        patches.append(patch.object(SFJenkins, 'run_script'))
        with nested(*patches) as (jenkins_open, run_script):
            jenkins_open.return_value = SAMPLE_JENKINS_JOB_DESC
    ...
    def test_run(self):
        patches = [
            patch.object(SFJenkins, 'jenkins_open'),
            patch.object(SFJenkins, 'get_job_info'),
        ]
        with nested(*patches) as (jenkins_open, get_job_info):
            jenkins_open.return_value = SAMPLE_JENKINS_JOB_DESC
    ...
    def test_stop(self):
        patches = [
            patch.object(SFJenkins, 'jenkins_open'),
            patch.object(SFJenkins, 'stop_build'),
        ]
        with nested(*patches) as (jenkins_open, stop_build, ):
            jenkins_open.return_value = SAMPLE_JENKINS_JOB_DESC
